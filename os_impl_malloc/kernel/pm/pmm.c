...
void* coalesce(void* this_block)
{
	...
	printk("previous block size: %d 0x%x\n", GET_SIZE(PREV_BLKP(HDRP(this_block))), PREV_BLKP(HDRP(this_block)));
...
void* extend_heap()
{
	...
	else
	{
		...
		printk("address is 0x%x %d\n", FTRP(block_ptr), GET_SIZE(FTRP(block_ptr)));
	...
	for(list_index = free_list_index; list_index < NUM_OF_LIST; list_index++)
	{
		...
		if(free_list[list_index] != NULL)
		{
			...
			if(block_size >= alloc_size && (block_size - alloc_size) > MIN_SIZE)
			{
				...
				alloc_ptr = (void*)((uint32_t*)alloc_ptr + 1);
				...
				PUT(FTRP(list_elem_ptr), PACK(remaining_size, 0));
				...
				if(remaining_size > 0)
				{
					...
					printk("allocate block size: %d 0x%x\n", GET_SIZE(HDRP(alloc_ptr)), HDRP(alloc_ptr));
					printk("allocate block size: %d 0x%x\n", GET_SIZE(FTRP(alloc_ptr)), FTRP(alloc_ptr));
					printk("remaining block size: %d 0x%x\n", GET_SIZE(HDRP(list_elem_ptr)), HDRP(list_elem_ptr));
					printk("remaining block size: %d 0x%x\n", GET_SIZE(FTRP(list_elem_ptr)), FTRP(list_elem_ptr));
