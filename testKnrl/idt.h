...
#define SET_IDT_GATE(a, x, y, z, w)	({	\
	((struct idt_entry*)a + (x))->base_low   = (y) & 0xFFFF;	\
	((struct idt_entry*)a + (x))->base_high  = ((y) >> 16) & 0xFFFF;	\
	((struct idt_entry*)a + (x))->selector   = (z);	\
	((struct idt_entry*)a + (x))->attr       = (w);	\
	((struct idt_entry*)a + (x))->always0    = 0;	\
})
...
void isr_handler_dispatcher(proc_ctx_t* this_context);
